...
% two flagella diffusive- motors and tubulin shared with constant disassembly
...
kon = (Lss/v+Lss^2/(2*D))^(-1)*(.5*gammakM*(T-2*Lss)/da-1); %value of k
gammaM = gammakM/kon;
...
%     end
    ...
    if ~isempty(find_b)
        motor(3, find_b) = motor(3, find_b) + dx; %finding motors which are ballistic and move them by a constant amount
    end
