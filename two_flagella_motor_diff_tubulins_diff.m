...
% two flagella diffusive- motors not shared tubulin not shared with constant disassembly
...
kon = (Lss/v+Lss^2/(2*D))^(-1)*(gammakM*(T-Lss)/da-1); %value of k
gammaM = gammakM/kon;
...
freeT1=T-len1(1); % amount of free tubulin in basal pool 1
...
%     end
    ...
    if rand<=prob_motor1
        ...
        motor1(4, mot1) = gamma*freeT1; %amount of tubulin the motor carries
    ...
    if ~isempty(find_td1)
        motor1(3,find_td1) =  2*len1(time+1) - motor1(3,find_td1);
    end
    
    find_ba = find(motor(3,:)<=0 & motor(1,:)==1 & motor(2,:)==2); %finding motors which have reached the basal pool 1
    if ~isempty(find_ba)
        motor(:,find_ba)=0;       
        freeM=freeM+length(find_ba);
    end
    
    find_ba1 = find(motor1(3,:)<=0 & motor1(1,:)==1 & motor1(2,:)==2); %finding motors which have reached the basal pool 2
    if ~isempty(find_ba1)
        motor1(:,find_ba1)=0;       
